...
1. **Назначение проекта** — 1–3 предложения (единственный обязательный).
...
Дописать (если нет): `.claude/worktrees/` и `.claude/settings.local.json`.

### `CLAUDE.md` проекта

Только предметный слой: назначение, стек, команды запуска, соглашения домена,
ссылки на `.claude/forge.md` и `.claude/harness.json`. Процесс не дублируй —
он приходит из плагина. Существующий CLAUDE.md не перезаписывай — предложи
дополнение отдельным блоком.
...
`docs/README.md` — карта документации. Для нового проекта — заготовки
